...
**Required query-string params on `DATABASE_URL`:** `?pgbouncer=true&connection_limit=1&pool_timeout=40`. Without `pgbouncer=true`, Prisma caches prepared statements that the pooler doesn't preserve across connections → P1017 "Server has closed the connection". `connection_limit=1` is intentional: Supabase free-tier session pooler caps at `pool_size=15` total across the whole account, and each Vercel serverless container opens its own Prisma pool — so anything higher than 1 will hit `FATAL: (EMAXCONNSESSION) max clients reached in session mode` under any real concurrency. With `connection_limit=1`, N warm containers = N conns, which stays well under 15. `pool_timeout=40` gives `Promise.all` fan-outs (dashboard, activity) time to serialize their queries through the single connection.
...
- `DATABASE_URL` **must** include `?pgbouncer=true&connection_limit=1&pool_timeout=40` — without `pgbouncer=true`, Prisma will fail at runtime with P1017.
